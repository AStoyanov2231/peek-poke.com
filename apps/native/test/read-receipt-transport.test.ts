...
describe("native read receipt transport", () => {
  it("commits only an exact validated 2xx response", async () => {
    const commit = vi.fn();
    const fetchMock = response({ success: true, last_read_sequence: 7 });
    ...
    expect(fetchMock).toHaveBeenCalledWith(
      `https://www.peek-poke.com/api/dm/${THREAD_ID}/read`,
...
function response(payload: unknown) {
  return vi.fn(async () => new Response(JSON.stringify(payload), {
    headers: { "x-request-id": "read-receipt-native" },
  }));
}
