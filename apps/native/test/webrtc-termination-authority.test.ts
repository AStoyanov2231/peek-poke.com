...
vi.mock("@/lib/secure-storage", () => ({
  secureStorage: {
    getItem: vi.fn(async (key: string) => mocks.secureValues.get(key) ?? null),
...
let hook: ReturnType<typeof useNativeWebRTCCall> | null = null;
...
describe(`native WebRTC terminal authority integration (${process.env.NATIVE_TEST_PLATFORM ?? "shared"})`, () => {
  ...
  it("reports a rejected pre-factory authority check once while the call is current", async () => {
    mocks.synchronize.mockRejectedValue(new Error("authority unavailable"));
