...
| `/group/[groupId]` - shared QR group chat | Shared QR group conversation; see `docs/shared-qr-groups.md` for the membership, messaging, and recovery contract | `apps/native/app/group/[groupId].tsx` provides the same conversation with native keyboard/back/safe-area behavior | Same Expo route and notification allowlist | Repository implementation is present; the migrations documented in `docs/shared-qr-groups.md`, isolated Supabase migration validation, staging, and browser/device proof are required before rollout |
...
| `GET/POST /api/groups` and `GET/POST /api/groups/[groupId]`, `POST /api/groups/[groupId]/read` | Shared QR group endpoints; see `docs/shared-qr-groups.md` for the opaque payload, authorization, idempotency, and rollout contract | Web and native scanners use the shared payload contract and both inboxes merge groups with DMs | Repository implementation is present; additive migration promotion, outbox worker deployment, staging, and browser/device proof remain required |
