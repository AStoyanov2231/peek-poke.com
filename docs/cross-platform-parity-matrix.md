...
| `/group/[groupId]` - shared QR group chat | Membership-authorized bounded history, text send with client idempotency, monotonic group unread state, realtime recovery, generic safe group label, loading/error/retry | `apps/native/app/group/[groupId].tsx` provides the same member-only text conversation and native keyboard/back/safe-area behavior | Same Expo route and notification allowlist | Repository implementation is present; `20260814000000_shared_qr_groups.sql`, isolated Supabase migration validation, staging, and browser/device proof are required before rollout |
...
| `GET/POST /api/groups` and `GET/POST /api/groups/[groupId]`, `POST /api/groups/[groupId]/read` | Exact opaque QR text is bounded and hashed server-side; atomic create/join, member-only history/send/read, idempotent text messages, and safe generic errors | Web and native scanners use the same exact payload contract and both inboxes merge groups with DMs | Repository implementation is present; additive migration promotion, outbox worker deployment, staging, and browser/device proof remain required |
