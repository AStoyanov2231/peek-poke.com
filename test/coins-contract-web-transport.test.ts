...
beforeEach(() => activateMeetingCompletionOwner(ACCOUNT_ID));
...
describe("web coins transports", () => {
  it.each([
    ...
    await expect(client.fetchQuery({ queryKey: webQueryKeys.coins, queryFn: () => fetchCoins() }))
      .rejects.toMatchObject({ code: "INVALID_RESPONSE", status: 502 });
    expect(client.getQueryData(webQueryKeys.coins)).toBeUndefined();
  });
  ...
  it.each([
    ...
    await expect(recordMeeting(ACCOUNT_ID, FRIEND_ID).then((result) => {
      if (!result.already_met) client.setQueryData(webQueryKeys.coins, { balance: result.balance });
      met.add(FRIEND_ID);
    })).rejects.toMatchObject({ code: "INVALID_RESPONSE", status: 502 });
    expect(client.getQueryData(webQueryKeys.coins)).toBeUndefined();
  ...
  it("coalesces web background/CTA overlap and preserves the mounted absolute-balance commit", async () => {
    let resolve!: (response: Response) => void;
    const fetchMock = vi.fn(() => new Promise<Response>((done) => { resolve = done; }));
    vi.stubGlobal("fetch", fetchMock);
    const client = clientWithoutRetries();
    const backgroundCommit = vi.fn((result: { balance: number }) => {
      client.setQueryData(webQueryKeys.coins, { balance: result.balance });
    });
    ...
    const cta = recordMeeting(
      ...
      undefined,
      ctaCommit,
      "web-chat-meeting:test",
    );
    unsubscribeMeetingAttempt(ACCOUNT_ID, OVERLAP_FRIEND_ID, "web-background-meeting:test");
    ...
    expect(client.getQueryData(webQueryKeys.coins)).toEqual({ balance: 4 });
  });
  ...
  it.each([
    ...
    "survives a capped-wallet web route remount for %s/%s order without a second request",
    async (...order) => {
      let resolve!: (response: Response) => void;
      const fetchMock = vi.fn(() => new Promise<Response>((done) => { resolve = done; }));
      vi.stubGlobal("fetch", fetchMock);
      ...
      // A new detector instance after route remount consults the app-lifecycle registry.
      ...
      const remountedCta = await recordMeeting(
        ...
        undefined,
        commit,
        "web-chat-meeting:remounted",
      );

      expect(fetchMock).toHaveBeenCalledOnce();
      const options = fetchMock.mock.calls[0]?.[1] as RequestInit;
      expect(new Headers(options.headers).get("idempotency-key")).toEqual(expect.any(String));
  ...
  it("fences a deferred web CTA/background response on direct unauthorized recovery and lets reauth retry", async () => {
    ...
    const background = recordMeeting(
      ...
      undefined,
      staleCommit,
      "web-background-meeting:stale-epoch",
    );
    const cta = recordMeeting(
      ACCOUNT_ID,
      OVERLAP_FRIEND_ID,
      undefined,
      staleCommit,
      "web-chat-meeting:stale-epoch",
    );
    await vi.waitFor(() => expect(fetchMock).toHaveBeenCalledTimes(1));

    const redirect = vi.fn();
    recoverUnauthorizedWebSession(redirect);
    ...
    await expect(recordMeeting(
      ...
      undefined,
      vi.fn(),
      "web-chat-meeting:stale-after-signout",
    ...
    const retry = recordMeeting(
      ...
      undefined,
      currentCommit,
      "web-chat-meeting:current-epoch",
    );
    ...
    expect(meetingPairCompleted(ACCOUNT_ID, OVERLAP_FRIEND_ID)).toBe(true);
    expect(client.getQueryData(webQueryKeys.coins)).toEqual({ balance: 6 });
  });

  it("fences deferred A on an observed web A to B to A auth transition without a B meeting", async () => {
    const resolvers: Array<(response: Response) => void> = [];
    const fetchMock = vi.fn(() => new Promise<Response>((done) => { resolvers.push(done); }));
    vi.stubGlobal("fetch", fetchMock);
  ...
  it("keeps a deferred web meeting current across an idempotent same-user token refresh", async () => {
    let resolve!: (response: Response) => void;
    const fetchMock = vi.fn(() => new Promise<Response>((done) => { resolve = done; }));
    vi.stubGlobal("fetch", fetchMock);
    const commit = vi.fn();
  ...
  it("fences a cached web preflight before its synthetic response can commit", async () => {
  ...
  it("rejects cold-start and mismatched web callers until auth explicitly activates them", async () => {
...
function response(payload: unknown) {
  return vi.fn(async () => new Response(JSON.stringify(payload), {
    headers: { "x-request-id": "coins-web-request" },
  }));
}
