...
import type { PhotoApprovalStatus, Profile, ProfilePhoto } from "@/types/database";
...
async function fetchPhotos(status: PhotoApprovalStatus, page: number) {
...
function PhotoGrid({
  ...
  const { data, isLoading, isError } = useQuery({
    ...
    queryFn: () => fetchPhotos(status, page),
              ...
              <div className="absolute bottom-0 left-0 right-0 p-2 bg-gradient-to-t from-black/70 to-transparent opacity-0 group-hover:opacity-100 transition-opacity">
                <div className="flex items-center gap-1.5">
                  <Avatar className="h-5 w-5 flex-shrink-0">
                    ...
                    <AvatarFallback className="text-[10px]">{photo.user.username?.[0]?.toUpperCase()}</AvatarFallback>
                  </Avatar>
      ...
      <Dialog open={!!viewerPhoto} onOpenChange={(open) => !open && setViewerPhoto(null)}>
        <DialogContent className="max-w-3xl p-0 bg-black/95 border-none">
          ...
          <div className="flex items-center justify-center min-h-[60vh] p-6">
            {viewerPhoto && (
              <img src={viewerPhoto.url} alt="" className="max-w-full max-h-[75vh] object-contain" />
            )}
          </div>
...
export function AdminModerationTab() {
  ...
  const prefetch = (s: PhotoApprovalStatus) => {
    queryClient.prefetchQuery({ queryKey: ["mod-photos", s, 1], queryFn: () => fetchPhotos(s, 1) });
  };
        ...
        <DialogContent>
          ...
          <DialogFooter>
            ...
            <Button
              ...
              onClick={() => rejectingPhotoId && rejectMutation.mutate({ photoId: rejectingPhotoId, reason: rejectionReason.trim() })}
            >
