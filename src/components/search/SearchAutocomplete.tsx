...
export function SearchAutocomplete({
  ...
  }, [isTagMode, tagSuggestions.length, nearby.length, others.length]);
  ...
  useEffect(() => {
    function handleKeyDown(e: KeyboardEvent) {
      ...
      if (e.key === 'Enter') {
        ...
        if (!entry) return;
        if (entry.type === 'tag') {
          onReplaceActiveTag({ name: entry.item.name });
        } else {
          onSelectUser(entry.item.id);
        }
      }
    }
    document.addEventListener('keydown', handleKeyDown);
    return () => document.removeEventListener('keydown', handleKeyDown);
  }, [flatItems, highlightedIndex, onClose, onSelectUser, onReplaceActiveTag]);
  ...
  return (
    <div
      ...
      className={cn(
        ...
        'bg-white/95 backdrop-blur-md border border-hairline rounded-lg shadow-lg overflow-hidden',
        className,
      )}
    >
      {isLoading ? (
