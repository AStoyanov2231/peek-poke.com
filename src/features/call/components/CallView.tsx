...
import { DraggableSelfView } from "@/features/call/components/DraggableSelfView";
import { CallControlBar } from "@/features/call/components/CallControls";
...
export function CallView({ call }: CallViewProps) {
  ...
  useEffect(() => {
    ...
    const t = setTimeout(() => useCallStore.getState().clearCall(call.callId, call.generation), 2_500);
    return () => clearTimeout(t);
  }, [status, call.callId, call.generation]);
  ...
  return (
    ...
    >
      ...
      {status === "connected" && !remoteStream ? null : (
        // Remote call audio is the product behavior; muting would make the
        // peer inaudible. Playback begins only after the user starts/accepts a call.
        // react-doctor-disable-next-line no-autoplay-without-muted
        <video
          ref={remoteVideoRef}
          autoPlay
          playsInline
        ...
        >
          <track kind="captions" src="data:text/vtt,WEBVTT" srcLang="en" label="Live call captions" />
        </video>
      )}
      ...
      >
        {/* Closing a call view also ends the peer session. */}
        <button type="button"
          onClick={endCall}
          disabled={isEnding}
          aria-label={isEnding ? "Ending call" : "End call and close"}
      ...
      {/* ── Bottom controls ── */}
      <CallControlBar
        micIcon={isMicMuted ? <MicOff size={22} /> : <Mic size={22} />}
        micLabel={isMicMuted ? "Unmute microphone" : "Mute microphone"}
        cameraIcon={isCameraOff ? <VideoOff size={22} /> : <Video size={22} />}
        cameraLabel={isCameraOff ? "Turn camera on" : "Turn camera off"}
