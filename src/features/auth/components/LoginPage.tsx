...
function LoginContent() {
  ...
  const safeRedirectTo =
    redirectTo && isSafeInternalRedirect(redirectTo) ? redirectTo : undefined;
  const safeInvite =
    invite && /^[a-zA-Z0-9-]+$/.test(invite) ? invite : undefined;
    ...
    return (
      <LazyMotion features={domAnimation}>
        ...
        >
          ...
          <p className="text-muted-foreground mb-6">
            We sent a confirmation link to your email. Click it to complete
            signup.
          </p>
          <motion.button
            {...hoverTap}
    ...
    <LazyMotion features={domAnimation}>
      ...
      >
        ...
        <form method="post" onSubmit={handleSubmit} className="space-y-3">
          ...
          <motion.div {...hoverTap}>
            <Button
              ...
              disabled={!hydrated || loading || oauthLoading !== null}
            >
              {loading ? (
        ...
        {mode === "signin" ? (
          <button
            type="button"
            disabled={!hydrated || resettingPassword}
