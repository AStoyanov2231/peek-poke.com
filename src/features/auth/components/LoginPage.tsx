...
// This component coordinates the complete authentication flow and its visual states.
// react-doctor-disable-next-line no-giant-component
function LoginContent() {
  const searchParams = useSearchParams();
  // These values are accepted only after strict internal-path/token validation.
  // react-doctor-disable-next-line url-prefilled-privileged-action
  const redirectTo = searchParams.get("redirectTo");
  const invite = searchParams.get("invite");
  const safeRedirectTo = redirectTo && isSafeInternalRedirect(redirectTo) ? redirectTo : undefined;
  const safeInvite = invite && /^[a-zA-Z0-9-]+$/.test(invite) ? invite : undefined;

  const [mode, setMode] = useState<"signin" | "signup">("signin");
  const [email, setEmail] = useState("");
  const [error, setError] = useState(
    searchParams.has("error") ? "Failed to authenticate. Please try again." : ""
  );
  ...
  const [passwordResetStatus, setPasswordResetStatus] = useState<string | null>(
    searchParams.has("passwordUpdated") ? "Your password was updated. Sign in with the new password." : null
  );
  ...
  async function handleAppleSignIn() {
    ...
    setError("");
    const next = safeInvite ? `/invite/${safeInvite}` : safeRedirectTo;
    ...
    try {
      ...
      setResendStatus(
        result.success
          ...
          : "We could not resend the email yet. Please wait a moment and try again."
      );
    } finally {
      setResending(false);
    }
    ...
    try {
      ...
      setPasswordResetStatus(
        result.success
          ...
          : "Enter a valid email address, or wait a moment before trying again."
      );
    } finally {
  ...
  if (emailNotConfirmed) {
    return (
      ...
      >
        ...
        {resendStatus ? (
          <p className="text-sm text-muted-foreground mb-4" aria-live="polite">{resendStatus}</p>
        ) : null}
        <motion.button
          {...hoverTap}
        ...
        <button
          ...
          className="mt-3 block w-full text-sm font-semibold text-primary"
        >
          Back to Sign In
        </button>
        </motion.div>
    ...
    >
      ...
      <form onSubmit={handleSubmit} className="space-y-3">
        <input
          type="hidden"
          name="redirectTo"
          value={safeInvite ? `/invite/${safeInvite}` : (safeRedirectTo || "")}
        />
        <InputWithIcon
          name="email"
          type="email"
