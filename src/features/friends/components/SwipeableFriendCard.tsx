...
export function SwipeableFriendCard({
  ...
  const handleEnd = useCallback(() => {
    ...
    const percentage = containerWidth > 0 ? Math.abs(translateX) / containerWidth : 0;
    ...
    // Reset the position
    setTranslateX(0);
  }, [containerWidth, isDragging, onSwipeComplete, translateX]);
  ...
  const swipePercentage = containerWidth > 0 ? Math.abs(translateX) / containerWidth : 0;
  ...
  return (
    <div
      ...
      role="group"
      aria-label="Swipeable friend card"
      onKeyDown={(event) => { if (event.key === "Escape") didSwipeRef.current = false; }}
