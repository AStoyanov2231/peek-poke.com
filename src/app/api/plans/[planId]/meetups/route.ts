...
import { planMeetupRequestSchema } from "@peekpoke/shared/plans";
...
export const GET = withAuth<Params>(async (_request, { user, params }) => {
  ...
  const result = await readPlanMeetupStatus(user.id, params.planId);
...
export const POST = withAuth<Params>(async (request, { user, params }) => {
  ...
  const [body, error] = await parseBody(request, planMeetupRequestSchema);
  if (error) return error;
  const result = await acknowledgePlanMeetup(user.id, params.planId, body, key.key);
  if ("unavailable" in result) return planUnavailable();
  if (result.failure) return planFailure(result.failure);
